    ...
    for (unsigned int i = 0; i < buffers.size(); ++i)
    {
        ...
        if (!buffers[i].queued)
        {
            ...
            if (rc)
            {
                ...
                printf("iKVM:VideoBuffer - Failed to queue buffer\n");
            }
            else
            {
...
bool Video::needsResize()
{
    ...
    if (rc < 0)
    {
        ...
        //                entry("ERROR=%s", strerror(errno)));
        printf("iKVM:VideoBuffer - Failed to query timings\n");
        restart();
        return false;
    }
    ...
    if (timings.bt.width != width || timings.bt.height != height)
    {
        ...
        if (!width || !height)
        {
            ...
            printf("iKVM:VideoBuffer - Failed to get new resolution\n");
        }
...
void Video::resize()
{
    ...
    if (needsResizeCall)
    {
        ...
        if (rc)
        {
            ...
            printf("iKVM:VideoBuffer - Failed to stop streaming\n");
        }
    }
    ...
    if (needsResizeCall)
    {
        ...
        if (rc < 0)
        {
            ...
            elog<ReadFailure>(
                ...
                xyz::openbmc_project::Common::Device::ReadFailure::
                    CALLOUT_DEVICE_PATH(path.c_str()));*/
            printf("iKVM:VideoBuffer - Failed to zero streaming buffers\n");
        }
        ...
        if (rc < 0)
        {
            ...
            elog<ReadFailure>(
                ...
                xyz::openbmc_project::Common::Device::ReadFailure::
                    CALLOUT_DEVICE_PATH(path.c_str()));*/
            printf("iKVM:VideoBuffer - Failed to set timings\n");
        }
    ...
    if (rc < 0 || req.count < 2)
    {
        ...
        elog<ReadFailure>(
            ...
            xyz::openbmc_project::Common::Device::ReadFailure::
                CALLOUT_DEVICE_PATH(path.c_str()));*/
        printf("iKVM:VideoBuffer - Failed to request streaming buffers\n");
    }
    ...
    for (i = 0; i < buffers.size(); ++i)
    {
        ...
        if (rc < 0)
        {
            ...
            elog<ReadFailure>(
                ...
                xyz::openbmc_project::Common::Device::ReadFailure::
                    CALLOUT_DEVICE_PATH(path.c_str()));*/
            printf("iKVM:VideoBuffer - Failed to query buffer\n");
        }
        ...
        if (buffers[i].data == MAP_FAILED)
        {
            ...
            elog<ReadFailure>(
                ...
                xyz::openbmc_project::Common::Device::ReadFailure::
                    CALLOUT_DEVICE_PATH(path.c_str()));*/
            printf("iKVM:VideoBuffer - Failed to mmap buffer\n");
        }
    ...
    if (rc)
    {
        ...
        elog<ReadFailure>(
            ...
            xyz::openbmc_project::Common::Device::ReadFailure::
                CALLOUT_DEVICE_PATH(path.c_str()));*/
        printf("iKVM:VideoBuffer - Failed to start streaming\n");
    }
}
...
void Video::start()
{
    ...
    {
    	//DEBUG
		printf("iKVM_DEBUG: 1_fd_open_failed\n");
		//DEBUG
        ...
        if (fd < 0)
        {
            ...
            elog<Open>(
                ...
                xyz::openbmc_project::Common::File::Open::PATH(path.c_str()));*/
            printf("iKVM:VideoBuffer - Failed to open video device\n");
        }
    }

    //DEBUG
	printf("iKVM_DEBUG: memset cap\n");
	//DEBUG
    ...
    if (rc < 0)
    {
        ...
        elog<ReadFailure>(
            ...
            xyz::openbmc_project::Common::Device::ReadFailure::
                CALLOUT_DEVICE_PATH(path.c_str()));*/
        printf("iKVM:VideoBuffer - Failed to query video device capabilities\n");
    }
        ...
        !(cap.capabilities & V4L2_CAP_STREAMING))
    {
        ...
        elog<Open>(
            xyz::openbmc_project::Common::File::Open::ERRNO(errno),
            xyz::openbmc_project::Common::File::Open::PATH(path.c_str()));*/
        printf("iKVM:VideoBuffer - Video device doesn't support this application\n");
    }

    //DEBUG
	printf("iKVM_DEBUG: memset fmt\n");
	//DEBUG
    ...
    if (rc < 0)
    {
        ...
        elog<ReadFailure>(
            ...
            xyz::openbmc_project::Common::Device::ReadFailure::
                CALLOUT_DEVICE_PATH(path.c_str()));*/
        printf("iKVM:VideoBuffer - Failed to query video device format\n");
    }

    //DEBUG
	printf("iKVM_DEBUG: memset sparm\n");
	//DEBUG
    ...
    if (rc < 0)
    {
        /*log<level::WARNING>("Failed to set video device frame rate",
                            entry("ERROR=%s", strerror(errno)));*/
        printf("iKVM:VideoBuffer - Failed to set video device frame rate\n");
    }
    ...
    //DEBUG
	printf("iKVM_DEBUG: enter resize\n");
	//DEBUG
